...
class AWSdump:
    ...
    def dump_data(self):
        ...
        for device_id in devices:
            ...
            last_sample = self.traces.data[self.traces.data['device_id']==device_id]['cloud_t'].iloc[-1]
            ...
            if self.params['max_gap']<(cloud_t-last_sample):
                ...
                json_local_path, mseed_local_path, json_s3_path, mseed_s3_path = self.get_name_and_path(trace)
                ...
                self.traces.data = self.traces.data[self.traces.data['device_id']!=device_id]
    ...
    def get_name_and_path(self, trace):
        ...
        mseed_filename = self.params["network"] + "." + device_id + "." + timestamp.strftime("%Y.%j.%H.%M.%S") + ".MSEED"
        
        json_local_path = "./tmp/jsonl/" + json_filename
        mseed_local_path = "./tmp/mseed/" + mseed_filename

        json_s3_path = "test_traces/json/dev" + device_id + "/yr" + year + "/jd" + jd + "/" + json_filename
    ...
    def json2mseed(self, json_local_path, mseed_local_path):
        ...
        with open(json_local_path) as f:
            ...
            for entry in f:
                ...
                tr.stats.starttime = UTCDateTime(record['cloud_t']) - (len(record['x']) - 1) / record['sr']
                tr.stats.station = record["device_id"]
                tr.stats.network = network

                if len(tr.stats.station) > 4:
                    warn("Station name for {} now {} to fit MSEED format".format(tr.stats.station, tr.stats.station[0:4]))
